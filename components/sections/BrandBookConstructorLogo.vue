      ...
      <div class="constructor__create-logo">
        ...
        <form class="constructor__form">
          ...
          <client-only>
            ...
            <div class="constructor__wrap-btns">
              <button v-if="!isSafari" type="button" class="button-light left-icon" @click.prevent="downloadAsPng">
                <svg class="button-light__img" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="24" height="24.053" viewBox="0 0 24 24.053">
                  <defs>
                    <clipPath id="clip-path">
                      <rect width="24.053" height="24" fill="none" stroke="#0e0e10" stroke-width="1.5"></rect>
                    </clipPath>
                  </defs>
                  <g transform="translate(24) rotate(90)" clip-path="url(#clip-path)">
                    <path d="M22,1h1.553a3,3,0,0,1,3,3V20.5a3,3,0,0,1-3,3H22" transform="translate(-3.249 -0.25)" fill="none" stroke="#0e0e10" stroke-miterlimit="10" stroke-width="1.5"></path>
                    <line y1="0.004" x2="18.433" transform="translate(0 11.987)" fill="none" stroke="#0e0e10" stroke-miterlimit="10" stroke-width="1.5"></line>
                    <g transform="translate(-3.019 12.76)">
                      <line transform="translate(21.591 -0.761)" fill="none" stroke="#0e0e10" stroke-miterlimit="10" stroke-width="1.5" opacity="0"></line>
                      <path d="M20,20.331l5-5-5-5" transform="translate(-2.482 -16.092)" fill="none" stroke="#0e0e10" stroke-miterlimit="10" stroke-width="1.5"></path>
                    </g>
                  </g>
                </svg>
                <span class="button-light__text">Скачать PNG</span>
              </button>
              <button type="button" class="button-light left-icon"  @click.prevent="downloadAsSvg">
                <svg class="button-light__img" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="24" height="24.053" viewBox="0 0 24 24.053">
                  <defs>
                    <clipPath id="clip-path">
                      <rect width="24.053" height="24" fill="none" stroke="#0e0e10" stroke-width="1.5"></rect>
                ...
                <span class="button-light__text">Скачать SVG</span>
              </button>
            </div>
          </client-only>
        </form>
      </div>
      <fieldset class="constructor__logo-border">
        <legend class="constructor__title-logo">Как нельзя использовать логотип</legend>
        <div class="constructor__inner-logos">
          <div class="constructor__logo-box">
            <!--  TODO +++ я бы обернул картинку в div и задал бы фикс высоту -->
            <div class="constructor__inner-logo">
              <img src="@/assets/img/brandbook/logos/logo-white.svg" alt="">
            </div>
            <p class="constructor__logo-description">Нельзя закрашивать логотип цветом, кроме фирменного чёрного.</p>
          </div>
          <div class="constructor__logo-box">
            <div class="constructor__inner-logo">
              <img src="@/assets/img/brandbook/logos/logo-violet.svg" alt="">
            </div>
            <p class="constructor__logo-description">Нельзя закрашивать логотип цветом, кроме фирменного белого.</p>
          </div>
          <div class="constructor__logo-box">
            <div class="constructor__inner-logo">
              <img src="@/assets/img/brandbook/logos/logo-green.svg" alt="">
            </div>
            <p class="constructor__logo-description">При наложении на фон – логотип не должен сливаться, и быть не
              ярко
              выраженным.</p>
          </div>
          <div class="constructor__logo-box">
            <div class="constructor__inner-logo">
              <img src="@/assets/img/brandbook/logos/logo-gray.svg" alt="">
            </div>
            <p class="constructor__logo-description">При наложении на фон – логотип не должен сливаться, и быть не
              ярко
              выраженным.</p>
...
export default {
  ...
  data() {
    return {
      ...
      maxWidth: 1200,
      heightRatio: 0.48,
      heightWithoutSloganPercent: 81.8,
      colorText: '#0e0e10',
      slogan: '1',
      bgColor: '',
      isNoBgColor: true,
  ...
  computed: {
    height() {
      if (this.slogan !== '1') {
        return (this.width*this.heightRatio/100)*this.heightWithoutSloganPercent;
      }
      return this.width*this.heightRatio;
    },
    widthText() {
      return `${this.width}x${(this.height).toFixed(2)}`;
    },
    widthRem() {
      const html = document.querySelector('html');
      if (!html) {
        return `${this.width/16}rem`; // 1rem = 16px
      }

      const htmlFontSize =  parseFloat(window.getComputedStyle(html, null).getPropertyValue('font-size'));
      return `${this.width/htmlFontSize}rem`;
    },
  },
  mounted() {
  ...
  watch: {
    ...
    bgColor(value) {
      if (value) {
        this.isNoBgColor = false;
      }
    }
  },
  methods: {
    getSvgString() {
      const svgDom = this.$refs.logo;
      if (!svgDom) {
        return '';
      }
    ...
    downloadAsSvg() {
      try {
        const blob = new Blob([this.getSvgString()]);

        if (!blob) {
          return;
        }
        const element = document.createElement("a");
        element.download = "logo-skillline.svg";
    ...
    downloadAsPng() {
      try {
        ...
        img.onload = function () {
          ...
          const element = document.createElement("a");
          element.download = "logo-skillline";
